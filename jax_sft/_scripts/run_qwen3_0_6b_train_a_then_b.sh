...
CNL_MASK_MODE="${CNL_MASK_MODE:-hard}"
CNL_MARGIN="${CNL_MARGIN:-0.0}"
CNL_LEAK="${CNL_LEAK:-0.0}"
...
B_RETENTION_SUBSET_MODE="${B_RETENTION_SUBSET_MODE:-random}"
if [[ -z "${SYNTHETIC_B_RETENTION:-}" ]]; then
  if [[ "${B_METHOD}" == *_synth ]]; then
...
echo "CNL_MASK_MODE     : ${CNL_MASK_MODE}"
...
echo "B_RETENTION_RATIO : ${B_RETENTION_RATIO}"
echo "B_RETENTION_SEED  : ${B_RETENTION_SEED}"
echo "B_RETENTION_SUBSET: ${B_RETENTION_SUBSET_MODE}"
